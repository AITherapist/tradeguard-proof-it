  ...
  try {
    ...
    return new Response(reportHTML, {
      headers: { 
        ...
        "Content-Disposition": `attachment; filename="Bluhatch-Report-${jobData.client_name.replace(/\s+/g, '_')}-${new Date().toISOString().split('T')[0]}.html"`
      },
      status: 200,
    });
    ...
    logStep("ERROR in generate-report", { message: errorMessage });
